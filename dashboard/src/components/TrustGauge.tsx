...
export const TrustGauge = ({ score }: { score: number }) => {
  ...
  return (
    <div className="relative flex items-center justify-center w-64 h-64 mx-auto" style={{ zIndex: 1 }}>
      {/* Outer Ring */}
      <svg className="w-full h-full transform -rotate-90" style={{ display: 'block' }}>
        <circle
          cx="128"
          cy="128"
          r="120"
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center" style={{ zIndex: 2 }}>
        <Shield size={48} color={color} className="mb-2 opacity-80 drop-shadow-lg" />
        <div className="text-5xl font-bold font-mono text-white drop-shadow-lg">
          {percentage.toFixed(0)}<span className="text-2xl text-gray-400">%</span>
        </div>
        <div className="text-xs uppercase tracking-[0.2em] text-gray-400 mt-2 font-bold">Trust Score</div>
      </div>
